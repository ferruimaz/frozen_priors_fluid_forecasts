# ============================================================
# Experiment C_ID Baselines — CIFAR-10 ID Experiments
...
def load_clip_fast_head():
    ...
    # Channel-wise (R,G,B) mean and std from CLIP's training data; inputs must be
    # normalized with these exact constants to match the pretrained model's expectations.
...
def run_id_experiment():
    """
    Run ID experiments on CIFAR-10 data only with Bayesian Bootstrap.
    """
    print(f"Setting up CIFAR-10 ID Baselines...")
    ddpm = load_ddpm_cifar10()
    clip_uncertainty, class_names = load_clip_fast_head()
    train, test = load_datasets()

    # Pools - all from CIFAR-10
    trial_pool_idx = list(range(len(train)-cfg.calib_size))
    truth = torch.stack([test[i][0] for i in range(cfg.truth_pool)])
    ...
    # Frequentist targets θ(F*) on held-out CIFAR-10 test
    print("Frequentist targets θ(F*) on CIFAR-10 test...")
    ...
    for n0 in cfg.n_list:
        ...
        for r in tqdm(range(cfg.R), desc=f"Trials n0={n0} [ID]", leave=True):
            # draw X0 from CIFAR-10 trial pool only
            pool_idx = np.random.choice(trial_pool_idx, size=n0, replace=False)
            X0 = torch.stack([train[i][0] for i in pool_idx])
    ...
    print(f"\nDone. [ID] Total time: {int(total//60)}m {int(total%60)}s  | Generations: {_generation_count}")

    # -------- Summary --------
    print("\n=== SUMMARY (q=%.2f) — ID (CIFAR-10) Baselines ===" % cfg.q)
    for nm in names:
        print(f"\n{nm}")
    ...
    out_path = f"expC_ID_baselines_{ts}.json"
